    ...
    {   
        ...
        {   
            ...
            if (simTime>=4)
            {
                ...
                // for (size_t i = 0; i < robotState.motors_pos_des.size(); i++)
                // {
                //     /* code */
                // std::cout<<res.err[i]<<", ";
                // }
            ...
            // robotState.motors_pos_des = {0,0.785398, 0 ,-1.5708 ,0, 0 ,0};
            // robotState.motors_pos_des = {0.0,0.0, 1.57, 0.0, 0.0, 0.0, 0.0};
            robotState.motors_vel_des = motors_vel_des;
            robotState.motors_tor_des = motors_tau_des;
            // printf("Site position (world frame): [%.4f, %.4f, %.4f]\n", pos[0], pos[1], pos[2]);
            // printf("Site rotation matrix (world frame):\n");
            // printf("[%.4f %.4f %.4f]\n", xmat[0], xmat[3], xmat[6]);
            // printf("[%.4f %.4f %.4f]\n", xmat[1], xmat[4], xmat[7]);
            // printf("[%.4f %.4f %.4f]\n", xmat[2], xmat[5], xmat[8]);

            // printWorldPose(mj_model,mj_data,"link1");
            // printWorldPose(mj_model,mj_data,"link2");
            // printWorldPose(mj_model,mj_data,"link3");
            // printWorldPose(mj_model,mj_data,"link4");
            // printWorldPose(mj_model,mj_data,"link5");
            // printWorldPose(mj_model,mj_data,"link6");
            // printWorldPose(mj_model,mj_data,"link7");
            // printWorldPose(mj_model,mj_data,"attachment_site",0);
            // std::cout<<"----------------------"<<std::endl;
            // printRelativePose(mj_model,mj_data,"base","link7");
